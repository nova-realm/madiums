'use client';

import Image from 'next/image';
import Link from 'next/link';
import type { Config } from '@/lib/types';
import { mdToHtml } from '@/lib/markdown';

const DISCORD_SVG = (
  <svg
    viewBox="0 0 24 24"
    fill="currentColor"
    style={{ width: 14, height: 14 }}
    aria-hidden
  >
    <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994.021-.041.001-.09-.041-.106a13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.929 1.793 8.18 1.793 12.061 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.893.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.028zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
  </svg>
);

const ARROW_SVG = (
  <svg
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
    style={{ width: 13, height: 13 }}
    aria-hidden
  >
    <line x1="5" y1="12" x2="19" y2="12" />
    <polyline points="12 5 19 12 12 19" />
  </svg>
);

interface ChangelogData {
  timestamp: string;
  title?: string;
  message: string;
  footerNote?: string;
}

interface Props {
  data: ChangelogData;
  config: Config;
}

export default function HomeChangelog({ data, config }: Props) {
  const avatarFile = config.footerAvatar.split('/').pop() || 'logo-support.png';
  const bodyHtml = mdToHtml(data.message);

  return (
    <div className="home-changelog-card">
      <div className="changelog-header">
        <div className="changelog-header-left">
          <span className="changelog-icon-wrap">{DISCORD_SVG}</span>
          <span className="changelog-header-title">Recent Updates</span>
        </div>
        <span className="changelog-status-pill">Latest</span>
      </div>

      <div className="changelog-discord-msg">
        <div className="dc-avatar-wrap">
          <Image
            src={`/assets/${avatarFile}`}
            alt={config.footerName}
            width={40}
            height={40}
            className="dc-avatar"
          />
        </div>

        <div className="dc-msg-content">
          <div className="dc-msg-meta">
            <span className="dc-msg-author">{config.footerName}</span>
            <span className="dc-msg-badge">DEV</span>
            <span className="dc-msg-time">{data.timestamp}</span>
          </div>

          <div
            className="dc-msg-body"
            dangerouslySetInnerHTML={{ __html: bodyHtml }}
          />

          {data.footerNote && (
            <div className="dc-msg-footer-note">
              <span className="dc-note-bar" />
              <p>{data.footerNote}</p>
            </div>
          )}

          <div className="dc-msg-actions">
            <Link href="/guide" className="changelog-quick-link">
              Read Support Guide {ARROW_SVG}
            </Link>
            <Link href="/qrs" className="changelog-quick-link secondary">
              Browse QRs {ARROW_SVG}
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
}
